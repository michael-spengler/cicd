...
export async function publishOnNestLand(): Promise<void> {
    ...
    let commandToBeExecuted = `deno run -A --unstable https://x.nest.land/eggs@0.3.1/eggs.ts link ${Deno.args[1]}`
    await CommandLineProcessor.process(commandToBeExecuted)

    commandToBeExecuted = `deno run -A --unstable https://x.nest.land/eggs@0.3.1/eggs.ts publish --version ${versionToBePublished} --no-check -Y`
    await CommandLineProcessor.process(commandToBeExecuted)
}
